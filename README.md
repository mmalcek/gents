...
- **Wire-accurate factory functions** alongside interfaces. `newItem()`
  produces a value that round-trips through `encoding/json` cleanly —
  including `time.Time` zeros (`'0001-01-01T00:00:00Z'`, not `''`),
  `[]byte` (base64 strings, not number arrays), and `int64` IDs wire-sent
  as strings when you use `json:",string"`.
...
- **Pure stdlib, ~700 LOC, no dependencies**. Install it, use it, forget
  it. Nothing to audit.
...
| `*T` | `T \| null` | `null` |
| Sibling `//gents:export` struct | stripped interface name | `new<Name>()` |
