...
Struct tags other than `json:` and `ts:` (gorm, binding, validate, xml,
msgpack, custom) coexist peacefully and are ignored. The `ts:` tag is a
per-field TS type override — see §3.13.
...
alongside `tFoo`/`newFoo()` is fine while `-map X=tFoo` is not.
...
- Header: exactly `// Code generated by github.com/mmalcek/gents; DO
  NOT EDIT.\n`
- Const blocks (§3.14) emit first, in scan order, one blank line before
  each block.
- One blank line before each struct.
- Every interface and const block is preceded by a JSDoc block ending
  in a `(source: file.go)` line (§3.12).
- Interface: `export interface <Name> {` ... `}\n`, fields one per
  line, 2-space indent. Documented fields get a JSDoc block at the
  same indent.
- One blank line between the interface and its factory.
- Factory: `export function new<Name>(): <Name> {\n  return {\n ...\n
  }\n}\n`, fields one per line, 4-space indent for object literal. No
  JSDoc on factories or inside factory bodies.
- Empty struct: `export interface <Name> {}\n` on a single line.
- Factory for all-optional struct: `return {}\n`.
- Exactly one trailing newline at EOF.

### 3.12 Doc comments & source provenance

Go doc comments travel into the output as JSDoc, so editor hover shows
the Go documentation at every SPA call site:

- **Structs**: the doc comment on the marked struct (`TypeSpec.Doc`,
  falling back to the declaration's doc for ungrouped `type Foo struct`
  decls). Grouped declarations don't inherit the group comment — it
  describes the group, not any one struct.
- **Fields**: the comment above the field, or the trailing same-line
  comment as fallback. Flattened embedded fields carry their original
  doc comments with them.
- **Consts**: the block comment plus each spec's comment (§3.14).

Directive lines (`//gents:export`, `//gents:map`, `//go:generate` and
friends) are stripped — `ast.CommentGroup.Text` drops comment
directives, so only human prose survives. A single doc line collapses
to the one-line `/** ... */` form; a literal `*/` inside a comment is
escaped so it cannot terminate the JSDoc block early.

Every interface and const block additionally ends its JSDoc with a
provenance line:
...
Deliberately the **basename only** — no directory, no line number.
Line numbers would make the generated file churn on every unrelated
edit above the struct (bad for committed generated files), and full
paths would vary between checkouts. The basename plus the struct name
is enough to jump to the declaration.

### 3.13 Per-field `ts:"..."` tag override

The `ts` struct tag replaces the emitted TS type for a single field:
...
- It is a **full escape hatch**: the Go field type is not consulted at
  ...
  With great power — gents cannot verify the tag matches the wire
  shape; that's the user's contract to keep.
- Scope is one field. For a type-wide mapping use `-map` /
...
### 3.14 Const-block export

`//gents:export` on a `const` declaration emits every named spec as an
`export const`:
...
- The marker must be on the `const` declaration itself. There is no
  per-spec marking — a block exports atomically or not at all (its
  specs usually only make sense together; bit flags are the canonical
  case). `_` names are skipped.
- **Supported values**: int/float/string literals, `true`/`false`,
  references to exported consts declared *earlier* in the input, unary
  `-` and `^` (emitted as `~`), parens, and the binary operators
  `+ - * & | ^ << >>`. String literals re-emit single-quoted; legacy
  `017` octal re-emits as decimal (a syntax error in TS modules
  otherwise).
- **Grouping is preserved with explicit parens** where needed: Go and
...
- **Panics** (all with `file:line` and the fix in the message):
  ...
  - Forward references — TS `const` declarations cannot
    forward-reference (temporal dead zone), so referenced consts must
    be declared earlier in the scanned input. (Go allows any order;
    TS doesn't. gents follows the stricter rule.)
  - `/`, `%`, `&^` — Go const division is exact/integer, JS division
    is float; `&^` has no TS spelling. Precompute the value.
  - Rune and imaginary literals, function calls, and anything else.
  - A const name colliding with a generated interface name or another
    exported const.
- **JS bitwise caveat**: TS/JS bitwise operators work on 32-bit
  integers. Bit-flag constants are fine; giant shifted masks
  (`1 << 40`) will be silently wrong in JS — precompute or avoid.
- Emission order: const blocks come first in the output, in scan
  order, before any interface (§3.11).
...
### The `-check` staleness gate

`-check` regenerates in memory and byte-compares against the existing
`-out` file: exit 0 when identical, exit 1 (with a "re-run gents"
message) when the file is stale or missing. Nothing is written, so it
composes safely with any CI pipeline:

```
go run github.com/mmalcek/gents/cmd/gents@<tag> -out ../shared/types.ts -check
```

This closes the one gap `//go:generate` leaves open: nothing otherwise
fails when someone edits a marked struct and forgets to regenerate —
tests pass against the stale `.ts` file and the drift ships. Edge case:
when the input has no marked structs *and* the output file doesn't
exist, `-check` exits 0 (nothing to generate, nothing on disk —
consistent).
...
embedded flattening, const export, JSDoc passthrough, `ts` tag,
`-check`). Remaining candidates for future releases:
